...
// G R B
byte colores[][3] = {{188, 194, 255}, {255, 0, 255}, {146, 187, 255}, {0, 255, 255} };
...
void fundir_a_color_led(byte led, byte icolor) {
  boolean r = true, g = true , b = true;
  byte gf = colores[icolor][0], rf = colores[icolor][1], bf = colores[icolor][2];
  byte ri, gi, bi;

  // comparamos el color actual con el nuevo para ver si tiene sentido el fundido.
  uint32_t color_actual = pixels.getPixelColor(led);
  uint32_t color_solicitado = pixels.Color(gf, rf, bf);
  ...
  if (color_actual != color_solicitado) {
    ...
    while (r || g || b) {
      ...
      pixels.setPixelColor(led, pixels.Color(gi, ri, bi));
      pixels.show();
      delay(1);
    }
  }
...
void efecto_leds(byte iefecto, byte icolor) {
  ...
  if (iefecto == 5) {
      byte led_random = random(100) % 2;
      byte color_random = random(0,total_colores);
      fundir_a_color_led(led_random, color_random);
  }
}
